...
class _OsmMapState extends State<_OsmMap> {
  ...
  void didUpdateWidget(_OsmMap old) {
    ...
    // Auto-fit when issue list changes
    if (widget.issues.length != old.issues.length && widget.issues.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _fitAll());
    }
    // Move to new center if it changed (e.g., user location)
  ...
  void _fitAll() {
    ...
    // Filter only issues with valid locations
    final validIssues = widget.issues.where((i) => i.hasValidLocation).toList();
    
    // Build list of points including all valid issues + user's current center
    final points = validIssues
        .map((i) => LatLng(i.location.latitude, i.location.longitude))
        .toList();
    
    // Always include the map center (user location) in the fit bounds
    points.add(widget.center);
    
    if (points.length == 1) {
      // Only user location or only one issue
